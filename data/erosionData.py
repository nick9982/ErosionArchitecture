...
class ErosionData:
    ...
    def loadImage(self, offset):
        ...
        outputImage = self.__extractChunk(
            imageio.imread(filepath.replace("Input", "Output"))
        )
    ...
    def saveImage(self, directory):
        ...
        imageio.imwrite(
            directory + self.images[self.currentImageIdx].replace("Input", "Output"),
            outputImage,
        )

    def placeDEMInOriginalAndSaveAsDem(self, destinationDirectory):
        input_1024 = imageio.imread(
            self.data_directory + self.images[self.currentImageIdx]
        )
        ...
        imageio.imwrite(
            destinationDirectory + self.images[self.currentImageIdx], r_channel
        )
